...
app.post('/analizar', upload.single('file'), async (req, res) => {
  ...
  try {
    ...
    if (!ultimoResumen) {
      const [rows] = await pool.query('SELECT resumen FROM resúmenes ORDER BY fecha DESC LIMIT 1');
...
// === Endpoint: Guardar resumen ===
app.post('/guardarResumen', async (req, res) => {
  try {
    const { resumen } = req.body;
    if (!resumen) return res.status(400).json({ error: 'Resumen requerido' });

    const sql = 'INSERT INTO resúmenes (resumen, fecha) VALUES (?, NOW())';
    await pool.query(sql, [resumen]);

    res.status(201).json({ message: 'Resumen guardado' });
  } catch (err) {
    console.error(err);
...
// === Endpoint: Historial de resúmenes ===
app.get('/historial', async (req, res) => {
  try {
    const [rows] = await pool.query('SELECT id, resumen, fecha FROM resúmenes ORDER BY fecha DESC');
    res.json(rows);
  } catch (err) {
    console.error(err);
    res.status(500).json({ error: err.message });
